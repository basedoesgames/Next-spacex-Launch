...
launch = requests.get("https://launchlibrary.net/1.4/launch?next=1&agency=121" ).json()['launches'][0]
...
last_update = str("Last updated: " + str(dt.month) + "/" + str(dt.day) + "/"+ str(dt.year) + " at " + str(dt.hour) + ":" + str(dt.minute) + " UTC")
...
Last Updated: %s/%s/%s (MM/DD/YYYY)  %s:%s (UTC-5) 
'''
#Only UTC-5 on PC, normal UTC on RPi

#Optional Local File Writing
#f= open("next_mission.txt","w")
#f.write(file_output % (launch['name'], launch['windowstart'], launch['windowend']))
#f.close()
...
repo.update_file("next_mission.txt", last_update, file_output % (launch['name'], launch['net'], launch['windowstart'], launch['windowend'], morelaunchinfo['name'], cm, cd, cy, cth, ctm), file.sha)
